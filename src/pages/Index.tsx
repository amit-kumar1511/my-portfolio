...
function Navbar({ menuOpen, setMenuOpen }: { menuOpen: boolean; setMenuOpen: (v: boolean) => void }) {
  return (
    <>
      ...
      {/* Mobile menu */}
      <div
        className={`fixed inset-x-4 top-20 z-40 glass-panel rounded-3xl p-6 transition-all duration-300 lg:hidden ${
      ...
      <div className="grid gap-8 lg:grid-cols-5">
        <div className="glass-panel rounded-3xl p-8 lg:col-span-3">
          <p className="text-base leading-relaxed text-on-surface-variant sm:text-lg">
            I am <span className="text-foreground font-semibold">Amit Kumar</span>, a Full Stack Developer and B.Tech CSE
            2026 student passionate about building responsive, scalable, and user-friendly web applications. I enjoy
            working with modern frontend and backend technologies and creating clean digital experiences.
          </p>
          <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3">
            {[
              ...
              <div key={s.v} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                <p className="font-display text-2xl font-bold text-orange">{s.k}</p>
                <p className="mt-1 text-xs text-on-surface-variant">{s.v}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {SKILLS.map((s) => {
          ...
          return (
            <div
              key={s.title}
              className="glass-panel group rounded-3xl p-7 transition-all duration-500 hover:-translate-y-1.5 hover:border-orange/30"
            >
    ...
    <section id="projects" className="scroll-mt-28 py-24 lg:py-32">
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
        {PROJECTS.map((p, i) => (
          <article
            key={p.title}
            className="glass-panel group flex flex-col overflow-hidden rounded-3xl transition-all duration-500 hover:-translate-y-1.5 hover:border-orange/30"
          >
            <div className="relative aspect-video overflow-hidden">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="material-symbols-outlined text-6xl text-white/30">
                  {["dashboard", "shopping_bag", "task_alt", "cloud", "lock", "smart_toy"][i]}
                </span>
              </div>
            ...
            <div className="flex flex-1 flex-col p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-1.5">
                {p.tags.map((t) => (
                  <span
                    key={t}
                    className="rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 font-code text-[10px] text-on-surface-variant"
                  >
                    {t}
                  </span>
                ))}
              ...
              <div className="mt-6 flex gap-2">
                <a
                  href="#"
                  className="btn-primary inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl px-4 py-2.5 font-display text-xs font-semibold"
                >
                  Live Demo
                ...
                <a
                  href="#"
                  className="btn-ghost inline-flex flex-1 items-center justify-center gap-1.5 rounded-xl px-4 py-2.5 font-display text-xs font-semibold"
                >
    ...
    <section id="certificates" className="scroll-mt-28 py-24 lg:py-32">
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        {CERTIFICATES.map((c) => (
          <div
            key={c.title}
            className="glass-panel group flex items-center gap-5 rounded-3xl p-6 transition-all duration-500 hover:-translate-y-1 hover:border-orange/30"
          >
    ...
    <section id="contact" className="scroll-mt-28 py-24 lg:py-32">
      ...
      <div className="grid gap-12 lg:grid-cols-12">
        ...
        <form
          ...
          className="glass-panel rounded-3xl p-8 lg:col-span-7"
        >
